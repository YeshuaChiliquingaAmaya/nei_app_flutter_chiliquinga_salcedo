...
import 'package:path_finder.dart';
import 'package:risk_predictor_service.dart';
import 'package:data_models.dart'; // Contiene las clases Node y Edge actualizadas
import 'package:floor_service.dart'; // Contiene la lógica para determinar el piso
...
class _MapScreenState extends State<MapScreen> {
  ...
  // --> 2. AÑADIMOS EL SERVICIO DE PISOS Y UNA VARIABLE PARA EL PISO ACTUAL
  ...
  Future<void> _loadMapData() async {
    ...
    for (var feature in features) {
      ...
      // --> 3. FILTRAMOS QUÉ CAMINOS Y MARCADORES MOSTRAR
      //     Mostraremos solo los del piso actual (_currentUserFloor)
      ...
      // Las zonas de riesgo se muestran siempre, sin importar el piso.
      if (type == 'Polygon') {
  ...
  Future<void> _startLocationTracking() async {
    ...
    try {
      ...
      // --> 4. DETERMINAMOS EL PISO INICIAL
      final initialFloor = _floorService.getFloorFromAltitude(initialPosition.altitude);
    ...
    _positionStreamSubscription = Geolocator.getPositionStream(locationSettings: locationSettings).listen((Position position) {
      ...
      // --> 5. DETERMINAMOS EL PISO EN CADA ACTUALIZACIÓN DE UBICACIÓN
      ...
      if (newFloor != _currentUserFloor) {
        ...
        _loadMapData(); // Si el usuario cambia de piso, redibujamos el mapa.
        _speak("Piso ${_currentUserFloor == 0 ? 'Bajo' : _currentUserFloor} detectado.");
      }
  ...
  void _calculateAndShowRoute({bool isRecalculation = false}) async {
    ...
    if (!isNavigating) {
      if (mounted) setState(() { isNavigating = true; });
    }
    ...
    // --> 6. PASAMOS EL PISO ACTUAL AL PATHFINDER
    final path = _pathFinder!.findShortestPath(startPoint, closestSafePoint.position, _currentUserFloor);
  ...
  void _updateNavigation(Position currentPosition) {
    ...
    final pathNodes = pathPoints.map((p) => _pathFinder!.findNearestNode(p)).where((n) => n != null).toList();

    if (_currentPathIndex >= pathNodes.length - 1) {
      final distanceToEnd = Geolocator.distanceBetween(userPoint.latitude, userPoint.longitude, pathNodes.last!.position.latitude, pathNodes.last!.position.longitude);
    ...
    if (distanceToNextWaypoint < _instructionThreshold && !_instructionGivenForCurrentIndex) {
      if (_currentPathIndex < pathNodes.length - 2) {
        ...
        // --> 7. LÓGICA PARA INSTRUCCIONES VERTICALES
        if (nextNode.floor != currentNode.floor) {
          final edge = _pathFinder!.adjacencyList[currentNode.id]!.firstWhere((e) => e.to == nextNode.id, orElse: () => Edge(from: 0, to: 0, baseCost: 0, distance: 0));
          if (edge.type == PathType.stairs) {
            ...
            if (mounted) setState(() { _instructionGivenForCurrentIndex = true; });
            return; // Evita dar una instrucción de giro al mismo tiempo
          }
        }

        // Lógica de giro existente
        final upcomingWaypoint = pathNodes[_currentPathIndex + 2]!.position;
        final bearing = Geolocator.bearingBetween(currentNode.position.latitude, currentNode.position.longitude, nextNode.position.latitude, nextNode.position.longitude);
        final nextBearing = Geolocator.bearingBetween(nextNode.position.latitude, nextNode.position.longitude, upcomingWaypoint.latitude, upcomingWaypoint.longitude);
  ...
  bool _isUserOffRoute(LatLng userPoint) {
    ...
    for (final point in _calculatedRoute!.points) {
      final d = Geolocator.distanceBetween(userPoint.latitude, userPoint.longitude, point.latitude, point.longitude);
  ...
  String _getTurnInstruction(double currentBearing, double targetBearing) {
    ...
    if (angle > 330 || angle < 30) return "continúe recto.";
    else if (angle >= 30 && angle < 150) return "gire a la derecha.";
    else if (angle >= 150 && angle < 210) return "dé la vuelta.";
    else return "gire a la izquierda.";
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        // --> 8. MOSTRAMOS EL PISO ACTUAL EN LA BARRA DE TÍTULO
        title: Text(isNavigating ? 'Piso: ${_currentUserFloor == 0 ? 'Bajo' : _currentUserFloor}' : 'Navegador de Evacuación'),
        actions: [
          IconButton(
